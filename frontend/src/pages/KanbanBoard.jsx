...
// 1. Memoized Card Component to prevent unnecessary rebuilds
const RepairCard = memo(({ repair, index, navigate, visibilityMutation }) => {
    return (
        <Draggable draggableId={repair.id.toString()} index={index}>
            {(provided, snapshot) => (
                <div
                    ...
                    className={`bg-white p-5 rounded-2xl shadow-sm border border-slate-200/60 group cursor-pointer relative overflow-hidden ${snapshot.isDragging
                        ? 'shadow-2xl border-accent/50 scale-[1.02] z-50'
                        : 'hover:shadow-xl hover:border-accent/30 hover:-translate-y-1 transition-all duration-300'
                        /* Notice: transition is ONLY applied when NOT dragging */
                        }`}
                >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <div className="w-fit h-7 px-2 pb-1 rounded-full bg-slate-900 border-2 border-white flex items-center justify-center text-xs font-black text-white ring-2 ring-slate-100">
...
const KanbanBoard = () => {
    ...
    // 2. Pre-calculate columns to avoid heavy filtering in the render loop
    const columns = useMemo(() => {
        if (!repairs) return {};
        return STAGES.reduce((acc, stage) => {
            acc[stage] = repairs.filter(r => r.status === stage);
            return acc;
        }, {});
    }, [repairs]);
    console.log(columns)
    const mutation = useMutation({
        ...
        onMutate: async ({ id, status, newIndex }) => { // 1. Accept newIndex
            ...
            queryClient.setQueryData(['repairs'], (old) => {
                ...
                const cleanList = old.filter(r => r.id.toString() !== id.toString());
                const movedItem = old.find(r => r.id.toString() === id.toString());

                if (!movedItem) return old;

                // 3. Create the updated object
                const updatedItem = { ...movedItem, status };

                // 4. Find all items already in the target stage to handle local indexing
                const itemsInTargetStage = cleanList.filter(r => r.status === status);
                const itemsInOtherStages = cleanList.filter(r => r.status !== status);

                // 5. Insert the item into the target stage array at the specific index
                const updatedTargetStage = [...itemsInTargetStage];
                updatedTargetStage.splice(newIndex, 0, updatedItem);

                return [...itemsInOtherStages, ...updatedTargetStage];
            });
    ...
    const onDragEnd = (result) => {
        ...
        // Pass the destination index to the mutation
        mutation.mutate({
            id: draggableId,
                        ...
                        <Droppable droppableId={stage}>
                            {(provided, snapshot) => (
                                <div
                                    ...
                                    ref={provided.innerRef}
                                    className={`flex flex-col gap-4 min-h-[600px] p-3 rounded-3xl border-2 border-dashed ${snapshot.isDraggingOver
                                        ? 'bg-slate-200/60 border-accent/30'
                                        : 'bg-slate-100/40 border-slate-200/60 shadow-inner'
                                        } ${!snapshot.isDraggingOver ? 'transition-colors duration-300' : ''}`}
                                /* Notice: We only allow transition-colors when NOT dragging over */
                                >
                                    {columns[stage]?.map((repair, index) => (
                                        <RepairCard
